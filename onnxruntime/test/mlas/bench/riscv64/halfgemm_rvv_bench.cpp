...
Options ParseArgs(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (key == "--m") options.m = std::strtoull(value.data(), nullptr, 10);
        else if (key == "--n") options.n = std::strtoull(value.data(), nullptr, 10);
        else if (key == "--k") options.k = std::strtoull(value.data(), nullptr, 10);
        ...
        else if (key == "--bias") options.use_bias = ParseBool(value);
    }
    return options;
}
